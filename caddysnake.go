...
func (f *CaddySnake) Provision(ctx caddy.Context) error {
	...
	f.app, err = NewPythonWorkerGroup(iface, module, f.WorkingDir, f.VenvPath, lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
	if err != nil {
		return err
	}
	...
	if f.Autoreload == "on" {
		...
		factory := func() (AppServer, error) {
			return NewPythonWorkerGroup(iface, module, f.WorkingDir, f.VenvPath, lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
		}
...
func (f *CaddySnake) provisionDynamic(workers int, cacheAddr string, startTimeout time.Duration) error {
	...
	factory := func(module, dir, venv string, envFiles []string, envVars map[string]string) (AppServer, error) {
		...
		return NewPythonWorkerGroup(iface, module, dir, venv, lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, logger)
	}

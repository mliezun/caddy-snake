...
func (f *CaddySnake) Provision(ctx caddy.Context) error {
	...
	if f.ModuleWsgi != "" {
		...
		f.app, err = NewPythonWorkerGroup("wsgi", f.ModuleWsgi, f.WorkingDir, f.VenvPath, f.Lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
		if err != nil {
			return err
		}
		if f.Lifespan != "" {
		...
		f.app, err = NewPythonWorkerGroup("asgi", f.ModuleAsgi, f.WorkingDir, f.VenvPath, f.Lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
		if err != nil {
			return err
		}
		...
		f.app, err = NewPythonWorkerGroup("esgi", f.ModuleEsgi, f.WorkingDir, f.VenvPath, "", rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
		if err != nil {
			return err
		}
		if f.Lifespan != "" {
	...
	if f.Autoreload == "on" {
		...
		if f.ModuleWsgi != "" {
			...
			factory = func() (AppServer, error) {
				return NewPythonWorkerGroup("wsgi", f.ModuleWsgi, f.WorkingDir, f.VenvPath, f.Lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
			}
		} else if f.ModuleAsgi != "" {
			rt := effectivePythonRuntime("asgi", f.Runtime)
			factory = func() (AppServer, error) {
				return NewPythonWorkerGroup("asgi", f.ModuleAsgi, f.WorkingDir, f.VenvPath, f.Lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
			}
		} else {
			rt := effectivePythonRuntime("esgi", f.Runtime)
			factory = func() (AppServer, error) {
				return NewPythonWorkerGroup("esgi", f.ModuleEsgi, f.WorkingDir, f.VenvPath, "", rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, f.logger)
			}
		}
...
func (f *CaddySnake) provisionDynamic(workers int, cacheAddr string, startTimeout time.Duration) error {
	...
	if f.ModuleWsgi != "" {
		...
		factory := func(module, dir, venv string, envFiles []string, envVars map[string]string) (AppServer, error) {
			...
			return NewPythonWorkerGroup("wsgi", module, dir, venv, lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, logger)
		}
		var err error
		...
		factory := func(module, dir, venv string, envFiles []string, envVars map[string]string) (AppServer, error) {
			pythonBin := resolvePythonInterpreter(pythonPath, venv)
			return NewPythonWorkerGroup("asgi", module, dir, venv, lifespan, rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, logger)
		}
		var err error
		...
		factory := func(module, dir, venv string, envFiles []string, envVars map[string]string) (AppServer, error) {
			pythonBin := resolvePythonInterpreter(pythonPath, venv)
			return NewPythonWorkerGroup("esgi", module, dir, venv, "", rt, workers, pythonBin, cacheAddr, envFiles, envVars, startTimeout, isolation, logger)
		}
		var err error
...
func NewPythonWorker(iface, app, workingDir, venv, lifespan, pyRuntime, pythonBin, scriptPath, cacheAddr, workerID string, envFiles []string, envVars map[string]string, startTimeout time.Duration, isolation *IsolationConfig, logger *zap.Logger) (*PythonWorker, error) {
	w := &PythonWorker{
		Interface:    iface,
		App:          app,
...
func NewPythonWorkerGroup(iface, app, workingDir, venv, lifespan, runtime string, count int, pythonBin, cacheAddr string, envFiles []string, envVars map[string]string, startTimeout time.Duration, isolation *IsolationConfig, logger *zap.Logger) (*PythonWorkerGroup, error) {
	...
	for i := 0; i < count; i++ {
		workers[i], errs[i] = NewPythonWorker(iface, app, workingDir, venv, lifespan, runtime, pythonBin, scriptPath, cacheAddr, strconv.Itoa(i), envFiles, envVars, startTimeout, isolation, logger)
	}
